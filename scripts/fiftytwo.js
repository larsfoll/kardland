...
fiftytwo.Stack.prototype._rectForCardAtIndex = function (index) {
  ...
  if (this.hSpread === 0.0 && this.vSpread === 0.0) {
    ...
    if (this.vPosition >= 0) {
      ...
      rect.top =
        Math.round(-this.vPosition * cardHeight) - cardHeight + index * 0.5;
    }
    ...
    if (offset) {
      ...
      if (this.vPosition >= 0) {
        ...
        rect.top =
          Math.round((this.vPosition + offset.top) * cardHeight) - cardHeight;
      }
...
fiftytwo.Stack.prototype._highlight = function (highlight) {
  "use strict";
  if (this._domElement) {
    this._domElement.style.backgroundColor = highlight
      ? "rgba(0,0,0,0.5)"
      : "transparent";
  }
};
  ...
  if (index >= 0) {
    ...
    if (this.hSpread === 0 && this.vSpread === 0) {
      ...
      if (this.vPosition >= 0) {
        ...
        cutStack.vPosition =
          (bounds.top - parentBounds.top) / this._tableContaining.cardHeight;
      }
    } else {
      offset = this._offsetForCardAtIndex(index);
      ...
      if (this.vPosition >= 0) {
        ...
        cutStack.vPosition =
          bounds.top / this._tableContaining.cardHeight + offset.top;
      }
    }
...
fiftytwo.Animation = function (actions, card, destStack) {
  ...
  if (actions.indexOf("move") >= 0) {
    ...
    if (1) {
      ...
      parentBounds =
        destStack._domElement.parentElement.getBoundingClientRect();
...
fiftytwo.Animation.prototype._easeInQuadFunction = function (
  frame,
  totalFrames,
  delta,
) {
...
fiftytwo.Animation.prototype._easeInEaseOutQuadFunction = function (
  frame,
  totalFrames,
  delta,
) {
...
fiftytwo.Animation.prototype._easeInEaseOutSinFunction = function (
  frame,
  totalFrames,
  delta,
) {
...
fiftytwo.Animation.prototype._execute = function (shadowElement) {
  ...
  if (this.actions.indexOf("move") >= 0) {
    ...
    if (complete) {
      ...
      easedProgress = this._easeInEaseOutSinFunction(
        this._progress,
        this._stepsTotal,
        this._xDistance,
      );
  ...
  if (this.actions.indexOf("flip") >= 0) {
    ...
    if (complete) {
      ...
      if (easedProgress < 0.5) {
        ...
        style =
          "perspective(8000) rotateY(" + (1.0 - easedProgress) * -180 + "deg)";
      }
...
fiftytwo.Table.prototype.addNewStack = function (
  identifier,
  role,
  hPos,
  vPos,
...
fiftytwo.Table.prototype._enqueueAnimation = function (animation) {
  ...
  for (i = 0; i < count; i += 1) {
    ...
    if (
      enequedAnimation.actions.indexOf("move") >= 0 &&
      enequedAnimation._card === card
    ) {
...
fiftytwo.Table.prototype.moveTopCardFromStackToStack = function (
  srcStack,
  destStack,
  animate,
) {
...
fiftytwo.Table.prototype.moveTopCardFromStackToStackAndFlip = function (
  srcStack,
  destStack,
  animate,
) {
...
fiftytwo.Table.prototype.performFunctionWhenLoadingComplete = function (
  completionFunction,
) {
  "use strict";
...
fiftytwo.Table.prototype.canDragStackFromStackToStack = function (
  stack,
  srcStack,
  destStack,
) {
...
fiftytwo.Table.prototype._allowedDragCardFromStack = function (card, stack) {
  ...
  return (
    !this._animating &&
    !this._draggingStack &&
    this.canDragCardFromStack(card, stack)
  );
};

// ---------------------------------------------------------------------- _beginDragCardFromStack ()

fiftytwo.Table.prototype._beginDragCardFromStack = function (
  event,
  card,
  stack,
) {
...
fiftytwo.Table.prototype._mouseMoved = function (event) {
  ...
  if (stackHit !== this._lastStackHit) {
    ...
    if (stackHit) {
      if (
        !this.canDragStackFromStackToStack(
          dragStack,
          this._sourceStack,
          stackHit,
        )
...
fiftytwo.Table.prototype._mouseUp = function (event) {
  ...
  if (stackHit) {
    if (
      !this.canDragStackFromStackToStack(dragStack, this._sourceStack, stackHit)
    ) {
      stackHit = null;
    }
  }

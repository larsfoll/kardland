...
function layoutTable() {
  ...
  // Create stacks.
  table
    .addNewStack("FREE1", "FREECELL", 0.0, 0.0, 0.0, 0.0)
    .setPlaceholderImage("images/freeCellPlaceholder.svg");
  table
    .addNewStack("FREE2", "FREECELL", 1.15, 0.0, 0.0, 0.0)
    .setPlaceholderImage("images/freeCellPlaceholder.svg");
  table
    .addNewStack("FREE3", "FREECELL", 2.3, 0.0, 0.0, 0.0)
    .setPlaceholderImage("images/freeCellPlaceholder.svg");
  table
    .addNewStack("FREE4", "FREECELL", 3.45, 0.0, 0.0, 0.0)
    .setPlaceholderImage("images/freeCellPlaceholder.svg");
  table
    .addNewStack("DIAMONDS", "FOUNDATION", 4.95, 0.0, 0.0, 0.0)
    .setPlaceholderImage("images/diamondFoundationPlaceholder.svg");
  table
    .addNewStack("CLUBS", "FOUNDATION", 6.1, 0.0, 0.0, 0.0)
    .setPlaceholderImage("images/clubFoundationPlaceholder.svg");
  table
    .addNewStack("HEARTS", "FOUNDATION", 7.25, 0.0, 0.0, 0.0)
    .setPlaceholderImage("images/heartFoundationPlaceholder.svg");
  table
    .addNewStack("SPADES", "FOUNDATION", 8.4, 0.0, 0.0, 0.0)
  ...
  if (stack.topCard() === card) {
    ...
    if (ordered) {
      ...
      if (emptyColumns > 0) {
        ...
        if (emptyColumns > 1) {
          unrestrictedNumberCanDrag =
            (emptyCells + 1) * ((emptyColumns - 1) * 2);
        } else {
        ...
        if (
          canDrag &&
          cardDragCount > unrestrictedNumberCanDrag &&
          !draggingEntireColumn
        ) {
          table.dragRestriction = dragRestrictions.DISALLOWEMPTYCOLUMN;
          showColumnPlaceholder(
            table.stacksWithRole("COLUMN"),
            "disallow",
            null,
          );
        }
...
function foundationShouldTakeCardSmart(card, table) {
  ...
  for (i = 0; i < count; i += 1) {
    ...
    if (!topCard) {
      ...
      if (
        card.color === cardSuit.RED &&
        card.rank <= lowestBlackFoundationRank + 1
      ) {
        stackToPutAwayTo = stack;
      } else if (
        card.color === cardSuit.BLACK &&
